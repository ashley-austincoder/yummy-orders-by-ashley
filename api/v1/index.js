...
app.get('/api/v1/orders', async (req, res) => {
  ...
  if (!user_id) {
    res
      .status(statusCodes.HTTP_UNPROCESSABLE)
  ...
  if (isNaN(Number(user_id))) {
    res
      ...
      .json({ errors: ['User Id is Invalid'] });
    return;
  }
  const normalizedDeliveryDate = normalizeDeliveryDate(delivery_date);
  const sql = getSql_AllMealsAndOrdersForUserId(
    user_id,
  ...
  if (!sql) {
    res
      .status(statusCodes.BAD_REQUEST)
      .json({ errors: ['Not enough information to retrieve orders'] });
    return;
  }
  ...
  try {
    ...
    res
      ...
      .json({ errors: ['Error retrieving orders'] });
  }
});
...
app.get('/api/v1/orders/getTotalCount', async (req, res) => {
  ...
  try {
    ...
    res
      .status(statusCodes.INTERNAL_SERVER_ERROR)
      .json({ errors: ['Error retrieving total order count'] });
  }
});
...
app.get('/api/v1/users', async (req, res) => {
  ...
  try {
    ...
    res
      .status(statusCodes.INTERNAL_SERVER_ERROR)
      .json({ errors: ['Error retrieving user'] });
  }
});
